...
function Parser(template, options) {
  this.template = cleantemplate(template);
...
function cleantemplate(template) {
  //replace \input syntax
  var inputs = template.match(/\\input{.{0,}?}/g);
  if (inputs != null) {
    inputs.forEach(function(m) {
      //first one gets name out of anchor, second trims whitespace
      template = template.replace(m, "<!> " + m.match(/(?!{)(\s*\w*)+(?=})/)[0].match(/\b.*\b/)[0] + ".tex !>");
    });
  }
